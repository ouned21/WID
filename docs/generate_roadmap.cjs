...
// ═══════════════════════════════════════════════════════════════════════
const toDevelop = [
  ['2.1', 'Onboarding conversationnel (4 questions)', 'Remplacer les 4-5 écrans actuels (équipements + famille + baseline + catalogue + thinking) par une conversation vocale/texte de 3 min : Yova pose 4 questions (composition foyer, qui fait quoi, équipements, projet à venir). Claude génère famille + 15 tâches + pack projet éventuel en une passe via route /api/ai/onboarding-bootstrap. Plus naturel, plus rapide, profil pré-rempli dès le départ.', 'À faire', 'P0', 'Post-tests Barbara · gros refactor'],
  ['2.2', 'Fil dans Journal (option A + B)', 'Pas d\'onglet Fil dédié. Intégrer la reconnaissance dans le Journal : A. Yova messagère — quand Barbara complète une tâche, Yova l\'annonce dans ton journal, tu réponds par ❤️/message libre → transmis à Barbara. B. Moments clés — Yova détecte et affiche des cartes ponctuelles ("Barbara a géré 5 tâches enfants cette semaine — 2× la moyenne"). 1-2 cartes/sem max, qualité > quantité.', 'À faire', 'P1', 'Post-tests Barbara'],
  ['2.3', 'Détection de routines', 'Yova observe : si tâches A+B+C complétées ensemble (même 2h, même personne, ≥3 fois), propose "Je regroupe en routine X ? 1 tap = les 3 marquées faites." Nouvelle table task_routines. Gain friction massif pour les routines quotidiennes (soir enfants, matin école).', 'À faire', 'P1', 'Post-tests Barbara'],
  ['2.4', 'SMS hebdo au partenaire non-installé', 'Option d\'invitation : user saisit nº téléphone partenaire, SMS opt-in RGPD-compliant ("réponds OUI/STOP"). Si OUI, SMS dimanche soir résumant la semaine reconnue. Canal social léger sans forcer l\'install.', 'À faire', 'P2', 'RGPD clause à rédiger dans CGU'],
  ['2.5', 'Wire up /api/ai/anticipate', 'Endpoint d\'anticipation proactive (tâches à venir, rappels). Route existe côté serveur. Brancher au Journal : Yova mentionne proactivement les tâches qui arrivent cette semaine.', 'À faire', 'P1', 'User insistait dessus'],
  ['2.6', 'Wire up /api/ai/insights', 'Insights IA hebdomadaires (version Premium de ce que DashboardFree fait côté client). Brancher au dashboard derrière gate Premium.', 'À faire', 'P2', ''],
  ['2.7', 'Wire up /api/ai/weekly-summary', 'Version IA enrichie du récap dimanche (actuellement stats pures côté client). Remplacer la carte récap dans /journal par appel Claude.', 'À faire', 'P2', ''],
  ['2.8', 'UX subtasks : refiner', 'La décomposition auto est active. Tester le comportement sur 5-10 tâches variées. Ajuster le seuil de Claude (3+ sous-tâches min). Ajouter analytics : combien de fois l\'écran s\'affiche, combien sont créées vs passées.', 'À faire', 'P2', ''],
];
